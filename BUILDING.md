...
We build ReeeKorder using [Microsoft Visual Studio 2019](https://visualstudio.microsoft.com/vs/community/). In order to build ReeeKorder **Desktop development with C++** workload is required.

As we require only C++14 - MSVC 2017 should work just fine too.

### MacOS

We build ReeeKorder using XCode 12. However, it is likely possible to build it with XCode 7.
...
1. Clone ReeeKorder from the ReeeKorder GitHub project. 
  
   For example, in the **git-bash** run:

    ```
    $ git clone https://github.com/possum64team/ReeeKorder/
    ```

2. Open CMake GUI. 
   
   Set the **Where is the source code** to the location where ReeeKorder was cloned. 
...
7. You can now run and debug ReeeKorder!
...
1. Clone Audacity from the ReeeKorder GitHub project. 
  
    ```
    $ git clone https://github.com/possum64team/ReeeKorder/
    ```

2. Configure ReeeKorder using CMake:
   ```
   $ mkdir build && cd build
   $ cmake -GXcode -T buildsystem=1 ../ReeeKorder
   ```

3. Open ReeeKorder XCode project:
   ```
   $ open Audacity.xcodeproj
   ```
   and build ReeeKorder using the IDE. 
...
```
cmake -GXCode -T buildsystem=1 -Daudacity_use_mad="off" -Daudacity_use_id3tag=off ../ReeeKorder
```
...
2. Configure ReeeKorder using CMake:
   ```
   $ mkdir build && cd build
   $ cmake -G "Unix Makefiles" -Daudacity_use_ffmpeg=loaded ../ReeeKorder
   ```
...
4. Testing the build:
   Adding a "Portable Settings" folder allows ReeeKorder to ignore the settings of any existing Audacity installation.
   ```
   $ cd bin/Debug
...
On Linux it is possible to build ReeeKorder using (almost) only the libraries provided by the package manager. Please, see the list of required libraries [here](linux/required_libraries.md).
...
1. **wxWidgets**: While ReeeKorder on **Linux** uses vanilla version of wxWidgets, we **require** that version **3.1.3** is used. This version is not available in most of the distributions.
2. **portaudio-v19**: ReeeKorder currently uses [some private APIs](https://github.com/audacity/audacity/issues/871), so using system portaudio is not yet possible.
