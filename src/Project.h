...
AUDACITY_DLL_API void SetActiveProject(ReeeKorderProject * project);
...
class AUDACITY_DLL_API AllProjects
{
   ...
   using AProjectHolder = std::shared_ptr< ReeeKorderProject >;
   using Container = std::vector< AProjectHolder >;
   static Container gReeeKorderProjects;
...
public:
   ...
   value_type Remove( ReeeKorderProject &project );
...
using AttachedProjectObjects = ClientData::Site<
   ReeeKorderProject, ClientData::Base, ClientData::SkipCopying, std::shared_ptr
>;
...
using AttachedProjectWindows = ClientData::Site<
   ReeeKorderProject, wxWindow, ClientData::SkipCopying, ClientData::BarePtr
>;
...
///\brief The top-level handle to an ReeeKorder project.  It serves as a source
...
class AUDACITY_DLL_API ReeeKorderProject final
   : public wxEvtHandler
   , public AttachedProjectObjects
   , public AttachedProjectWindows
   , public std::enable_shared_from_this<ReeeKorderProject>
{
 public:
...
inline wxFrame *FindProjectFrame( ReeeKorderProject *project ) {
   return project ? &GetProjectFrame( *project ) : nullptr;
}
inline const wxFrame *FindProjectFrame( const ReeeKorderProject *project ) {
   return project ? &GetProjectFrame( *project ) : nullptr;
}
