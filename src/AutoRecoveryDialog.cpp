...
{
public:
   explicit AutoRecoveryDialog(ReeeKorderProject *proj);
...
private:
   ...
   void OnQuitReeeKorder(wxCommandEvent &evt);
...
BEGIN_EVENT_TABLE(AutoRecoveryDialog, wxDialogWrapper)
   EVT_BUTTON(ID_QUIT_AUDACITY, AutoRecoveryDialog::OnQuitReeeKorder)
...
AutoRecoveryDialog::AutoRecoveryDialog(ReeeKorderProject *project)
...
void AutoRecoveryDialog::PopulateOrExchange(ShuttleGui &S)
{
   ...
   {
      S.AddFixedText(
         XO("The following projects were not saved properly the last time ReeeKorder was run and "
      ...
      {
         S.Id(ID_QUIT_AUDACITY).AddButton(XXO("&Quit ReeeKorder"));
...
bool AutoRecoveryDialog::HaveChecked()
{
   ...
   ReeeKorderMessageBox(XO("No projects selected"), XO("Automatic Crash Recovery"));

   return false;
}

void AutoRecoveryDialog::OnQuitReeeKorder(wxCommandEvent &WXUNUSED(evt))
{
                               ...
                               ReeeKorderProject *&pproj)
{
   ...
   for (auto &file: files)
   {
      ReeeKorderProject *proj = nullptr;
...
bool ShowAutoRecoveryDialogIfNeeded(ReeeKorderProject *&pproj, bool *didRecoverAnything)
{
