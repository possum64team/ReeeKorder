...
\brief Utility functions for working with our wxConf (gPrefs)
  ...
  ReeeKorder uses wxWidgets' wxConfig class to handle preferences.
...
\verbatim
  ...
  Preference field specification:
   /
      Version					- ReeeKorder Version that created these prefs
...
void EnumSettingBase::Migrate( wxString &value )
{
   ...
   if ( !mOldKey.empty() &&
      ...
      // ReeeKorder.  But further changes will be stored only to the NEW key
