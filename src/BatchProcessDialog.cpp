...
ApplyMacroDialog::ApplyMacroDialog(
   wxWindow * parent, ReeeKorderProject &project, bool bInherited):
...
void ApplyMacroDialog::OnApplyToProject(wxCommandEvent & WXUNUSED(event))
{
   ...
   if (item == -1) {
      ReeeKorderMessageBox(XO("No macro selected"));
      return;
   }
...
MacrosWindow::MacrosWindow(
   wxWindow * parent, ReeeKorderProject &project, bool bExpanded):
...
void MacrosWindow::OnAdd(wxCommandEvent & WXUNUSED(event))
{
   ...
   while (true) {
      ReeeKorderTextEntryDialog d(this,
...
/// Send changed values back to Prefs, and update ReeeKorder.
