...
/** Called during ReeeKorder start-up to try and load the ffmpeg libraries */
void FFmpegStartup()
{
   bool enabled = false;
...
\nbut this time ReeeKorder failed to load it at startup. \
...
public:
   ...
   void PopulateOrExchange(ShuttleGui & S)
   {
      ...
      S.StartVerticalLay(true);
      {
         S.AddTitle(
            XO(
"ReeeKorder needs the file '%s' to import and export audio via FFmpeg.")
               .Format( mName ) );
...
void FFmpegNotFoundDialog::PopulateOrExchange(ShuttleGui & S)
{
   ...
   {
      S.AddFixedText(XO(
"ReeeKorder attempted to use FFmpeg to import an audio file,\n\
