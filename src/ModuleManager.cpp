...
\brief Based on LoadLadspa, this code loads pluggable ReeeKorder 
extension modules.  It also has the code to
invoke a function returning a replacement window,
i.e. an alternative to the usual interface, for ReeeKorder.
...
void Module::ShowLoadFailureError(const wxString &Error)
{
   ...
   ReeeKorderMessageBox(XO("Unable to load the \"%s\" module.\n\nError: %s").Format(ShortName, Error),
...
bool Module::Load(wxString &deferredErrorMessage)
{
   ...
   if( moduleVersion != AUDACITY_VERSION_STRING) {
      ReeeKorderMessageBox(
         XO("The module \"%s\" is matched with ReeeKorder version \"%s\".\n\nIt will not be loaded.")
            .Format(ShortName, moduleVersion),
         XO("Module Unsuitable"));
      wxLogMessage(wxT("The module \"%s\" is matched with ReeeKorder version \"%s\". It will not be loaded."), mName, moduleVersion);
      mLib->Unload();
      return false;
   }
...
{
   const auto &audacityPathList = FileNames::ReeeKorderPathList();
   FilePaths pathList;
   wxString pathVar;
   ...
   const FilePaths &files, FilePaths &decided, DelayedErrors &errors)
{
   ...
   for (const auto &file : files) {
      ...
      // and let the user decide for each one.
      {
         ...
         int action;
         action = ShowMultiDialog(msg, XO("ReeeKorder Module Loader"),
            buttons,
            "",
