...
\class TrackPanel::ReeeKorderTimer
...
TrackPanel &TrackPanel::Get( ReeeKorderProject &project )
{
   return project.AttachedWindows::Get< TrackPanel >( sKey );
}

const TrackPanel &TrackPanel::Get( const ReeeKorderProject &project )
{
   return Get( const_cast< ReeeKorderProject & >( project ) );
}

void TrackPanel::Destroy( ReeeKorderProject &project )
{
...
/// Gets the pointer to the ReeeKorderProject that
/// goes with this track panel.
ReeeKorderProject * TrackPanel::GetProject() const
{
...
void TrackPanel::OnTimer(wxTimerEvent& )
{
   ...
   ReeeKorderProject *const p = GetProject();
...
struct EmptyCell final : CommonTrackPanelCell {
   std::vector< UIHandlePtr > HitTest(
      const TrackPanelMouseState &, const ReeeKorderProject *) override
   { return {}; }
...
IsVisibleTrack::IsVisibleTrack(ReeeKorderProject *project)
   : mPanelRect {
