...
UIHandle::Result TrackPanelResizeHandle::Click(
   const TrackPanelMouseEvent &evt, ReeeKorderProject *pProject )
{
...
UIHandle::Result TrackPanelResizeHandle::Cancel(ReeeKorderProject *pProject)
{
   auto &tracks = TrackList::Get( *pProject );
   auto pTrack = tracks.Lock(mpTrack);
   if ( !pTrack )
