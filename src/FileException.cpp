...
TranslatableString FileException::ErrorMessage() const
{
   ...
   switch (cause) {
      case Cause::Open:
         format = XO("ReeeKorder failed to open a file in %s.");
         break;
      case Cause::Read:
         format = XO("ReeeKorder failed to read from a file in %s.");
         break;
      case Cause::Write:
         return WriteFailureMessage(fileName);
      case Cause::Rename:
         format =
XO("ReeeKorder successfully wrote a file in %s but failed to rename it as %s.");
      default:
         break;
   }
...
{
   return XO("ReeeKorder failed to write to a file.\n"
