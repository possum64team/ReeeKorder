...
\brief Based on ThemeBase, Theme manages image and icon resources.
   ...
   Theme adds the ReeeKorder specific images to ThemeBase.
...
// Must be wide enough for bmpReeeKorderLogo. Use double width + 10.
...
//             .Format( FileName );
      ...
      if( !ImageCache.SaveFile( FileName, wxBITMAP_TYPE_PNG ))
      {
         ReeeKorderMessageBox(
            XO("ReeeKorder could not write file:\n  %s.")
               .Format( FileName ));
         return;
      }
      ReeeKorderMessageBox(
/* i18n-hint: A theme is a consistent visual style across an application's
 ...
 such as those on button controls.  ReeeKorder can load and save alternative
 themes. */
         XO("Theme written to:\n  %s.")
            .Format( FileName ));
   }
   ...
   else
   {
      ...
      {
         ReeeKorderMessageBox(
            XO("ReeeKorder could not open file:\n  %s\nfor writing.")
               .Format( FileName ));
         return;
      }
      ...
      {
         ReeeKorderMessageBox(
            XO("ReeeKorder could not write images to file:\n  %s.")
               .Format( FileName ));
         return;
      }
      ReeeKorderMessageBox(
...
//   }
   ...
   if(  type == themeFromFile )
   {
      ...
      if( !wxFileExists( FileName ))
      {
         ...
         ReeeKorderMessageBox(
            XO("ReeeKorder could not find file:\n  %s.\nTheme not loaded.")
               .Format( FileName ));
         return false;
      }
      if( !ImageCache.LoadFile( FileName, wxBITMAP_TYPE_PNG ))
      {
         ReeeKorderMessageBox(
            ...
            XO("ReeeKorder could not load file:\n  %s.\nBad png format perhaps?")
               .Format( FileName ));
         return false;
      }
   }
   ...
   else
   {
      ...
      if( !ImageCache.LoadFile( InternalStream, wxBITMAP_TYPE_PNG ))
      {
         ...
         ReeeKorderMessageBox(
            XO(
"ReeeKorder could not read its default theme.\nPlease report the problem."));
         return false;
      }
         ...
         if( wxFileExists( FileName ))
         {
            if( !mImages[i].LoadFile( FileName, wxBITMAP_TYPE_PNG ))
            {
               ReeeKorderMessageBox(
                  XO(
               /* i18n-hint: Do not translate png.  It is the name of a file format.*/
"ReeeKorder could not load file:\n  %s.\nBad png format perhaps?")
                     .Format( FileName ));
               return;
            }
...
"Some required files in:\n  %s\nwere already present. Overwrite?")
               .Format( FileNames::ThemeComponentsDir() ),
            ReeeKorderMessageBoxCaptionStr(),
   ...
   for(i = 0; i < (int)mImages.size(); i++)
   {
      if( (mBitmapFlags[i] & resFlagInternal)==0)
      {
         ...
         {
            ReeeKorderMessageBox(
               XO("ReeeKorder could not save file:\n  %s")
                  .Format( FileName ));
            return;
         }
      }
   ...
   wxT("/GUI/Theme"),
   {
      ByColumns,
      {
         /* i18n-hint: describing the "classic" or traditional
            appearance of older versions of ReeeKorder */
         XO("Classic")  ,
