...
const int ReeeKorderProjectTimerID = 5200;

static ReeeKorderProject::AttachedObjects::RegisteredFactory sProjectManagerKey {
   []( ReeeKorderProject &project ) {
      return std::make_shared< ProjectManager >( project );
   }
};

ProjectManager &ProjectManager::Get( ReeeKorderProject &project )
{
   return project.AttachedObjects::Get< ProjectManager >( sProjectManagerKey );
}

const ProjectManager &ProjectManager::Get( const ReeeKorderProject &project )
{
   return Get( const_cast< ReeeKorderProject & >( project ) );
}

ProjectManager::ProjectManager( ReeeKorderProject &project )
   : mProject{ project }
   , mTimer{ std::make_unique<wxTimer>(this, ReeeKorderProjectTimerID) }
{
...
// PRL:  This event type definition used to be in ReeeKorderApp.h, which created
...
BEGIN_EVENT_TABLE( ProjectManager, wxEvtHandler )
   ...
   EVT_TIMER(ReeeKorderProjectTimerID, ProjectManager::OnTimer)
END_EVENT_TABLE()
...
void ProjectManager::SaveWindowSize()
{
   ...
   else
   {
      if (foundIconizedProject) {
         ...
         // ReeeKorder would have to be running with no projects open
...
{
public:
   DropTarget(ReeeKorderProject *proj)
   {
      mProject = proj;
...
private:
   ...
   extern void AddPages(   ReeeKorderProject * pProj, GuiFactory & Factory,  wxNotebook  * pNotebook );
#endif
...
void InitProjectWindow( ProjectWindow &window )
{
   ...
   // LLL: When ReeeKorder starts or becomes active after returning from
   ...
   auto msg = XO("Welcome to ReeeKorder version %s")
...
ReeeKorderProject *ProjectManager::New()
{
   wxRect wndRect;
   ...
   auto sp = std::make_shared< ReeeKorderProject >();
   AllProjects{}.Add( sp );
   auto p = sp.get();
   auto &project = *p;
   ...
   //   SetDropTarget(safenew ReeeKorderDropTarget(this));
   //   mTrackPanel->SetDropTarget(safenew ReeeKorderDropTarget(this));
...
void ProjectManager::OnCloseWindow(wxCloseEvent & event)
{
   ...
   // ReeeKorderProject::~ReeeKorderProject() and TrackPanel::OnTimer().
   ...
   if (!sbSkipPromptingForSave 
      ...
      if ( UndoManager::Get( project ).UnsavedChanges() ) {
         ...
         int result = ReeeKorderMessageBox(
            Message,
            Title,
   ...
   // before the TrackPanel is destroyed. This change was needed to stop ReeeKorder
...
void ProjectManager::OpenFiles(ReeeKorderProject *proj)
{
   auto selectedFiles =
...
bool ProjectManager::SafeToOpenProjectInto(ReeeKorderProject &proj)
{
...
ReeeKorderProject *ProjectManager::OpenProject(
   ReeeKorderProject *pGivenProject, const FilePath &fileNameArg,
