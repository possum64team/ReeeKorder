...
used throughout ReeeKorder into this one place.
...
const FileNames::FileType
     ...
     /* i18n-hint an ReeeKorder project is the state of the program, stored as
     files that can be reopened to resume the session later */
   , FileNames::ReeeKorderProjects{ XO("AUP3 project files"), { wxT("aup3") }, true }
...
// The APP name has upercase first letter (so that Quit ReeeKorder is correctly
// capitalised on Mac, but we want lower case APP name in paths.
// This function does that substitution, IF the last component of
// the path is 'ReeeKorder'.
wxString FileNames::LowerCaseAppNameInPath( const wxString & dirIn){
   wxString dir = dirIn;
   // BUG 1577 Capitalisation of ReeeKorder in path...
   if( dir.EndsWith( "ReeeKorder" ) )
   {
      int nChars = dir.length() - wxString( "ReeeKorder" ).length();
...
FilePath FileNames::HtmlHelpDir()
{
   ...
   //for mac this puts us within the .app: ReeeKorder.app/Contents/SharedSupport/
...
wxFileNameWrapper FileNames::DefaultToDocumentsFolder(const wxString &preference)
{
   ...
   if ( bIsDefaultPath )
   {
      ...
      // The dialog that ReeeKorder offers will allow the user to select a valid directory.
...
static FilePaths sReeeKorderPathList;

const FilePaths &FileNames::ReeeKorderPathList()
{
   return sReeeKorderPathList;
}

void FileNames::SetReeeKorderPathList( FilePaths list )
{
   sReeeKorderPathList = std::move( list );
}
