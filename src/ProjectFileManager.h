...
{
public:
   static ProjectFileManager &Get( ReeeKorderProject &project );
   static const ProjectFileManager &Get( const ReeeKorderProject &project );

   // Open and close a file, invisibly, removing its Autosave blob
   static void DiscardAutosave(const FilePath &filename);

   explicit ProjectFileManager( ReeeKorderProject &project );
    ...
    *   are built into this build of ReeeKorder, each with the relevant file
   ...
   using ProjectChooserFn = std::function<ReeeKorderProject&(bool)>;
    ...
    */
   static ReeeKorderProject *OpenFile( const ProjectChooserFn &chooser,
...
private:
   /*!
    ...
    @return if something was successfully opened, the project containing it; else null
    */
   ReeeKorderProject *OpenProjectFile(
