...
\brief This is an ReeeKorder Specific ruler panel which additionally
...
public:
   QuickPlayIndicatorOverlay(ReeeKorderProject *project);
...
public:
   ...
   HitTestPreview DefaultPreview
      (const TrackPanelMouseState &state, const ReeeKorderProject *pProject)
      override
   {
   ...
   unsigned DoContextMenu
      (const wxRect &rect,
       wxWindow *pParent, wxPoint *pPosition, ReeeKorderProject*) final override
   {
...
protected:
   Result Click
      (const TrackPanelMouseEvent &event, ReeeKorderProject *) override
   {
   ...
   Result Drag
      (const TrackPanelMouseEvent &, ReeeKorderProject *) override
   {
      return RefreshCode::DrawOverlays;
   }

   Result Release
      (const TrackPanelMouseEvent &event, ReeeKorderProject *,
       wxWindow *) override
   {
   ...
   void Enter(bool, ReeeKorderProject *) override
   {
...
wxCoord GetPlayHeadX( const ReeeKorderProject *pProject )
{
...
double GetPlayHeadFraction( const ReeeKorderProject *pProject, wxCoord xx )
{
   const auto &viewInfo = ViewInfo::Get( *pProject );
   auto width = viewInfo.GetTracksUsableWidth();
...
public:
   ...
   HitTest( const ReeeKorderProject *pProject, wxCoord xx )
   {
...
public:
   ...
   std::vector<UIHandlePtr> HitTest
      (const TrackPanelMouseState &state,
       const ReeeKorderProject *pProject) override;
...
(const TrackPanelMouseState &state,
 const ReeeKorderProject *pProject)
{
...
namespace{
ReeeKorderProject::AttachedWindows::RegisteredFactory sKey{
[]( ReeeKorderProject &project ) -> wxWeakRef< wxWindow > {
...
AdornedRulerPanel &AdornedRulerPanel::Get( ReeeKorderProject &project )
{
   return project.AttachedWindows::Get< AdornedRulerPanel >( sKey );
}

const AdornedRulerPanel &AdornedRulerPanel::Get( const ReeeKorderProject &project )
{
   return Get( const_cast< ReeeKorderProject & >( project ) );
}

void AdornedRulerPanel::Destroy( ReeeKorderProject &project )
{
...
AdornedRulerPanel::AdornedRulerPanel(ReeeKorderProject* project,
                                     wxWindow *parent,
                                     wxWindowID id,
                                     const wxPoint& pos,
                                     const wxSize& size,
...
(const TrackPanelMouseEvent &event, ReeeKorderProject *pProject) -> Result
{
   auto result = CommonRulerHandle::Click(event, pProject);
   if (!( result & RefreshCode::Cancelled )) {
      if (mClicked == Button::Left) {
...
(ReeeKorderProject *pProject) -> Result
{
   auto result = CommonRulerHandle::Cancel(pProject);
...
{
   // This is called by ReeeKorderProject to make the play region follow
...
ReeeKorderProject * AdornedRulerPanel::GetProject() const
{
   return mProject;
}
