...
unsigned operator()
   ( const TrackPanelMouseEvent &evt, ReeeKorderProject *pProject ) const
{
   ...
   else if (event.CmdDown())
   {
         ...
         // using ReeeKorderProject zooming, which is smarter,
         ...
         ReeeKorderProject *p = GetProject();
         if( steps > 0 )
...
ReeeKorderProject::AttachedWindows::RegisteredFactory sProjectWindowKey{
...
ProjectWindow &ProjectWindow::Get( ReeeKorderProject &project )
{
   return project.AttachedWindows::Get< ProjectWindow >( sProjectWindowKey );
}

const ProjectWindow &ProjectWindow::Get( const ReeeKorderProject &project )
{
   return Get( const_cast< ReeeKorderProject & >( project ) );
}

ProjectWindow *ProjectWindow::Find( ReeeKorderProject *pProject )
{
   return pProject
...
const ProjectWindow *ProjectWindow::Find( const ReeeKorderProject *pProject )
{
   return Find( const_cast< ReeeKorderProject * >( pProject ) );
}
...
//yet checked in to ReeeKorder SVN as of 12-Feb-2010
   ...
   : ProjectWindowBase{ parent, id, pos, size, project }
{
   ...
   /* i18n-hint: This is an experimental feature where the main panel in
      ReeeKorder is put on a notebook tab, and this is the name on that tab.
   ...
   // flicker when ReeeKorder starts up.
...
void ProjectWindow::FixScrollbars()
{
   ...
   // of an ReeeKorderProject reopened from disk.
...
void ProjectWindow::OnShow(wxShowEvent & event)
{
   ...
   // (From Debian...see also TrackPanel::OnTimer and ReeeKorderTimer::Notify)
   //
   ...
   //  operation, causing reentry. ReeeKorder had a workaround in place
...
void ProjectWindow::OnActivate(wxActivateEvent & event)
{
   ...
   // ReeeKorder from a different application.
   //
   // This was observed by minimizing all windows using WINDOWS+M and
   // then ALT+TAB to return to ReeeKorder.  Focus will be given to the
...
ProjectWindow::PlaybackScroller::PlaybackScroller(ReeeKorderProject *project)
: mProject(project)
{
