...
TrackPanelAx::TrackPanelAx( ReeeKorderProject &project )
   :
...
wxAccStatus TrackPanelAx::GetLocation( wxRect& rect, int elementId )
{
   ...
   else
   {
      ...
      // Inflate the screen reader's rectangle so it overpaints ReeeKorder's own
...
TrackFocus &TrackFocus::Get( ReeeKorderProject &project )
{
   return project.AttachedObjects::Get< TrackFocus >( key );
}

const TrackFocus &TrackFocus::Get( const ReeeKorderProject &project )
{
   return Get( const_cast< ReeeKorderProject & >( project ) );
}

TrackFocus::TrackFocus( ReeeKorderProject &project )
   : mProject{ project }
{
}
