...
static ProjectFileIORegistry::Entry registerFactory{
   wxT( "tags" ),
   []( ReeeKorderProject &project ){ return &Tags::Get( project ); }
};

static const ReeeKorderProject::AttachedObjects::RegisteredFactory key{
  [](ReeeKorderProject &){ return std::make_shared< Tags >(); }
};

Tags &Tags::Get( ReeeKorderProject &project )
{
   return project.AttachedObjects::Get< Tags >( key );
}

const Tags &Tags::Get( const ReeeKorderProject &project )
{
   return Get( const_cast< ReeeKorderProject & >( project ) );
}

Tags &Tags::Set( ReeeKorderProject &project, const std::shared_ptr< Tags > &tags )
{
   auto &result = *tags;
