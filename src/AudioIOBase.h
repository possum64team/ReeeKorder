...
{
   explicit
   AudioIOStartStreamOptions(ReeeKorderProject *pProject_, double rate_)
      : pProject{ pProject_ }
      , envelope(nullptr)
      , rate(rate_)
      , playLooped(false)
...
public:
   ...
   void SetCaptureMeter(ReeeKorderProject *project, MeterPanelBase *meter);
   void SetPlaybackMeter(ReeeKorderProject *project, MeterPanelBase *meter);
    ...
    * for them (by resampling in reeekorder if needed) */
...
protected:
   ...
   ReeeKorderProject    *mOwningProject;
